<?php
class Block_BlkArticleCategoryHome2 extends Zend_View_Helper_Abstract{

	public function blkArticleCategoryHome2($template = 'default'){
		$view  = $this->view;
		...
		if($flagShow == true){
			...
			foreach ($row as $key => $val){
				...
				$select_item = $db->select()
					->from('articles AS a',array('id','name','alias','synopsis','picture','thumb','cat_id','like_fb','comment_fb','created'))
			...
			if(count($row) > 0){
				require_once (BLOCK_PATH . '/BlkArticleCategoryHome2/'.$template.'.php');
			}
		}
	}
}