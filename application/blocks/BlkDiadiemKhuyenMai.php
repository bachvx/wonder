<?php
class Block_BlkDiadiemKhuyenMai extends Zend_View_Helper_Abstract{

	public function BlkDiadiemKhuyenMai($template = 'default'){
		$view  = $this->view;
		...
		if($flagShow == true){
			...
			$select = $db->select()
				->from('diadiem AS p',array('id','name','alias','picture','thumb','synopsis','price','selloff','cat_id','units_money','khuyenmai'))
			...
			if(count($row) > 0){
				require_once (BLOCK_PATH . '/BlkDiadiemKhuyenMai/'.$template.'.php');
			}
		}
	}
}