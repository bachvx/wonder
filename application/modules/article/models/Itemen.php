<?php
class Article_Model_Itemen extends Zend_Db_Table{
	
	protected $_name = 'articles_en';
	...
	public function getItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'public-category'){
			if(!empty($arrParam['cid'])){
				...
				//$db = Zend_Db::factory($adapter, $config);
				$select = $db->select()
				->from('article_category AS ac',array('id','name','alias','meta_title','meta_description','meta_keywords'))
	...
	public function countItem($arrParam = null, $options = null){
		...
		if($options == null){
			...
			//$db = Zend_Db::factory($adapter, $config);
			$select = $db->select()
			->from('articles_en AS a', array('COUNT(a.id) AS totalItem'));
	...
	public function saveItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-add'){
            ...
            $row->id 				= stripslashes($arrParam['id_article']);
