<?php
	...
	$album_id = '';
	if(isset($this->arrParam['product_id'])){
		$album_id = '/product_id/' . $this->arrParam['product_id'];
	}
	...
	// 9. Back - Link
	$linkBackAlbum = $this->baseUrl ( 'product/admin-item/index');
	$btnBackAlbum = $this->cmsButton ( 'Back', $linkBackAlbum, $this->imgUrl . '/toolbar/icon-32-back.png', 'link' );
	...
	switch ($this->arrParam['action']){
		...
		case 'index': $strBtn = $btnAddNew . $divider . $btnActiveItems . ' ' . $btnInactiveItems . ' ' . $btnSortItem . $divider . $btnDeleteItems . ' ' . $btnBackAlbum;
					 			break;
