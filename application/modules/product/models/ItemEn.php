<?php
class Product_Model_ItemEn extends Zend_Db_Table{
	
	protected $_name = 'diadiem_en';
	...
	public function getItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'public-category'){
			if(!empty($arrParam['cid'])){
				...
				//$db = Zend_Db::factory($adapter, $config);
				$select = $db->select()
				->from('diadiem_category AS pc',array('id','name','alias','picture','picture_multi','content','meta_title','meta_description','meta_keywords'))
...
// 			}
			...
			if(!empty($arrParam['keywords'])){
				$select->where("(p.name like '%" . $arrParam['keywords'] . "%')");
			}
...
// 		}
	...
	public function listItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-list'){
			...
			$select = $db->select()
			->from('diadiem AS p',array('id','code','name','alias','picture','thumb','price_to','price','selloff','units_money','cat_id','order','hits','status','lang_code'))
		...
		if($options['task'] == 'public-category'){
		  //var_dump($arrParam);die;
			if(!empty($arrParam['cid'])){
				...
				$select = $db->select()
				->from('diadiem AS p',array('id','code','name','alias','synopsis','picture','thumb','cat_id','price','units_money'))
                ->where('p.manu_id = ?',$arrParam,INTERGER) 
				->where('p.status = ?',1,INTERGER)
				->order('id DESC');
              //  echo $select;die;
			}
...
// 			}
	...
	public function saveItem($arrParam = null, $options = null){
		if($options['task'] == 'admin-add'){
			...
			$row->id 				= $arrParam['id_product'];
			$row->code 				= $arrParam['code'];
			$row->name 				= stripslashes($arrParam['name_en']);
			$row->alias 			= $alias;
			...
			$row->synopsis			= stripslashes($arrParam['synopsis_en']);
			$row->content 			= stripslashes($arrParam['content_en']);
			...
			$row->made_in 	= $arrParam['made_in_en'];
			$row->baohanh	 	= $arrParam['baohanh_en'];
