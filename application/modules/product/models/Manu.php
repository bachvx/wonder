<?php
class Product_Model_Manu extends Zend_Db_Table{
	...
	public function itemInSelectbox($arrParam = null, $options = null){
		...
		if($options['task'] == 'get-sources'){
			$select = $db->select()
			->from('diadiem_manu AS pm',array('id','name'));
			$result = $db->fetchPairs($select);
			$result[0] = ' -- Chọn thương hiệu -- ';
			ksort($result);
		}
	...
	public function countItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-list'){
			...
			//$db = Zend_Db::factory($adapter, $config);
			$select = $db->select()
			->from('diadiem_manu AS pm', array('COUNT(pm.id) AS totalItem'));
	...
	public function listItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'public-list'){
			...
			if(!empty($arrParam['cid']) > 0){
				...
				->from('diadiem AS p',array('id','manu_id'))
				...
				if($arrParam['action'] == 'ban-chay'){
					$select_item->where('p.block_dongkhach = ?',1,INTERGER);
				}
