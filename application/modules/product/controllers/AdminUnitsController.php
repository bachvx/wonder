<?php
class Product_AdminUnitsController extends Zendvn_Controller_Action{
	...
	public function indexAction(){
		$this->view->Title = 'Đơn vị tính:: Danh sách';
		$this->view->headTitle($this->view->Title, true);
		
		$tblUnits = new Diadiem_Model_Units();
		$this->view->Items = $tblUnits->listItem($this->_arrParam, array('task'=>'admin-list'));
		
		$totalItem = $tblUnits->countItem($this->_arrParam, array('task'=>'admin-list'));
	...
	public function addAction(){
		$this->view->Title = 'Đơn vị tính:: Thêm mới';
		$this->view->headTitle($this->view->Title, true);
		...
		if($this->_request->isPost()){
			...
			$validator = new Diadiem_Form_ValidateUnits($this->_arrParam);
			if($validator->isError() == true){
				$this->view->messageError = $validator->getMessageError();
				$this->view->Item = $validator->getData();
			}else{
				$arrParam = $validator->getData();
				$tblUnits->saveItem($arrParam,array('task'=>'admin-add'));
				$this->_redirect($this->_actionMain);
			}
		}
	}
	...
	public function infoAction(){
		...
		$this->view->headTitle($this->view->Title, true);
		$tblUnits = new Hotel_Model_Item();
		$this->view->Item = $tblUnits->getItem($this->_arrParam,array('task'=>'admin-info')); */
	}
	
	public function editAction(){
		$this->view->Title = 'Đơn vị tính:: Sửa';
		$this->view->headTitle($this->view->Title, true);
		
		$tblUnits = new Diadiem_Model_Units();
		$this->view->Item = $tblUnits->getItem($this->_arrParam,array('task'=>'admin-edit'));
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
				...
				$arrParam = $validator->getData();
				$tblUnits->saveItem($arrParam,array('task'=>'admin-edit'));
	...
	public function deleteAction(){
		$this->view->Title = 'Đơn vị tính:: Xóa';
		$this->view->headTitle($this->view->Title, true);
		if($this->_request->isPost()){
			if(($this->_arrParam['type'] == 'multi-delete') && ($this->_arrParam['task'] == 'ok')){
				$tblUnits = new Diadiem_Model_Units();
				$tblUnits->deleteItem($this->_arrParam, array('task'=>'admin-delete-muti'));
				$this->_redirect($this->_actionMain . $this->_page);
			}else
				if(!empty($this->_arrParam['id'])){
				$tblUnits = new Diadiem_Model_Units();
				$tblUnits->deleteItem($this->_arrParam, array('task'=>'admin-delete'));
				$this->_redirect($this->_actionMain . $this->_page);
			}
		}
	}
	...
	public function statusAction(){
		$tblUnits = new Diadiem_Model_Units();
		$tblUnits->changeStatus($this->_arrParam);
		$this->_redirect($this->_actionMain . $this->_page);
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function sortAction(){
		if($this->_request->isPost()){
			$tblUnits = new Diadiem_Model_Units();
			$tblUnits->sortItem($this->_arrParam, array('task'=>'admin-sort'));
			$this->_redirect($this->_actionMain . $this->_page);
		}
		$this->_helper->viewRenderer->setNoRender();
	}
