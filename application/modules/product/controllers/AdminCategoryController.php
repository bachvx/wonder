<?php
class Product_AdminCategoryController extends Zendvn_Controller_Action{
	...
	public function addAction(){
		...
		$this->view->headTitle($this->view->Title, true);
		$tblCategory = new Product_Model_Category();
        $tblPageview = new Product_Model_Pageview();
		$this->view->slbCategory = $tblCategory->itemInSelectbox($this->_arrParam);
        $this->view->slbPage = $tblCategory->listItem($this->_arrParam, array('task'=>'get-sources'));
		
		if($this->_request->isPost()){
			$validator = new Product_Form_ValidateCategory($this->_arrParam);
			if($validator->isError() == true){
				...
				$arrParam['id_category']=$tblCategory->saveItem($arrParam,array('task'=>'admin-add'));
                //var_dump($id_category);die;
                //$arrParam['id_category']=$id_category;
                //$this->saveItem->lastInsertValue;
                $tblPageview->saveItem($arrParam,array('task'=>'admin-add'));
				$this->_redirect($this->_actionMain);
			}
		}
	}
	...
	public function editAction(){
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
                ...
                $tblPageview->saveItem($arrParam,array('task'=>'admin-edit'));
