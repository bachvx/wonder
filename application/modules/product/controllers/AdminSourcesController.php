<?php
class Product_AdminSourcesController extends Zendvn_Controller_Action{
	...
	public function indexAction(){
		...
		$this->view->Items = $tblSources->listItem($this->_arrParam, array('task'=>'admin-list'));
		
		$totalItem = $tblSources->countItem($this->_arrParam, array('task'=>'admin-list'));
	...
	public function addAction(){
		...
		if($this->_request->isPost()){
			...
			$validator = new Diadiem_Form_ValidateSources($this->_arrParam);
			if($validator->isError() == true){
				$this->view->messageError = $validator->getMessageError();
				$this->view->Item = $validator->getData();
			}else{
				$arrParam = $validator->getData();
				$tblSources->saveItem($arrParam,array('task'=>'admin-add'));
				$this->_redirect($this->_actionMain);
			}
		}
	}
	...
	public function infoAction(){
		...
		$this->view->headTitle($this->view->Title, true);
		$tblSources = new Hotel_Model_Item();
		$this->view->Item = $tblSources->getItem($this->_arrParam,array('task'=>'admin-info')); */
	}
	...
	public function editAction(){
		...
		$tblSources = new Diadiem_Model_Sources();
		$this->view->Item = $tblSources->getItem($this->_arrParam,array('task'=>'admin-edit'));
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
				...
				$arrParam = $validator->getData();
				$tblSources->saveItem($arrParam,array('task'=>'admin-edit'));
	...
	public function deleteAction(){
		...
		if($this->_request->isPost()){
			if(($this->_arrParam['type'] == 'multi-delete') && ($this->_arrParam['task'] == 'ok')){
				$tblSources = new Diadiem_Model_Sources();
				$tblSources->deleteItem($this->_arrParam, array('task'=>'admin-delete-muti'));
				$this->_redirect($this->_actionMain . $this->_page);
			}else
				if(!empty($this->_arrParam['id'])){
				$tblSources = new Diadiem_Model_Sources();
				$tblSources->deleteItem($this->_arrParam, array('task'=>'admin-delete'));
				$this->_redirect($this->_actionMain . $this->_page);
			}
		}
	}
	...
	public function statusAction(){
		$tblSources = new Diadiem_Model_Sources();
		$tblSources->changeStatus($this->_arrParam);
		$this->_redirect($this->_actionMain . $this->_page);
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function sortAction(){
		if($this->_request->isPost()){
			$tblSources = new Diadiem_Model_Sources();
			$tblSources->sortItem($this->_arrParam, array('task'=>'admin-sort'));
			$this->_redirect($this->_actionMain . $this->_page);
		}
		$this->_helper->viewRenderer->setNoRender();
	}
