<?php
class Product_PublicController extends Zendvn_Controller_Action {
	...
	public function init(){
		...
		$this->loadTemplate($template_path, 'template.ini', 'diadiem_public');
	...
	public function viewCartAction(){		
		...
		$this->view->Items = $tblDiadiem->listItem($this->_arrParam,array('task'=>'view-cart'));
	}
...
public function orderAction(){
		...
		$this->view->Title = $this->view->language['diadiemGuiThongTinDatHang'];
		$this->view->headTitle($this->view->Title, true);
		...
		if(count($this->view->Items) < 1){
			$this->_redirect('product/public/view-cart');
		}
		...
		if($this->_request->isPost()){
			...
			$validator = new Product_Form_ValidateOrder($this->_arrParam);
			if($validator->isError() == true){
               ...
               // die("1");
				...
				$tblInvoice = new Product_Model_Invoice();
				...
				$tblInvoiceDetail = new Product_Model_InvoiceDetail();
				$this->_arrParam['invoice_id'] = $invoice_id;
				$tblInvoiceDetail->saveItem($this->_arrParam);
				$yourCart->unsetAll();
				
				$this->_redirect('product/public/view-cart/save/ok');
			}
	...
	public function guideAction(){
		$this->view->title = $this->view->language['diadiemHuongDanDatCho'];
		$this->view->headTitle($this->view->title, true);
	}
	...
	public function tagsAction(){
		...
		$this->view->Items = $tblDiadiem->listItem($this->_arrParam,array('task'=>'public-filter', 'filter' => 'tags'));
		$this->view->totalItem = $totalItem = $tblDiadiem->countItem($this->_arrParam,array('task'=>'public-filter', 'filter' => 'tags'));
