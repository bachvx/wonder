<?php
class Product_AdminItemController extends Zendvn_Controller_Action{
	...
	public function indexAction(){
		...
		$this->view->Title = 'Sản phẩm:: Danh sách';
	...
	public function addAction(){
		$this->view->Title = 'Sản phẩm:: Thêm mới';
		$this->view->headTitle($this->view->Title, true);
		...
		$this->view->slbPage = $tblCategory->listItem($this->_arrParam, array('task'=>'get-phankhu'));
		
		if($this->_request->isPost()){
				
			$validator = new Product_Form_ValidateItem($this->_arrParam);
			if($validator->isError() == true){
				$this->view->messageError = $validator->getMessageError();
				$this->view->Item = $validator->getData();
			}else{
				$arrParam = $validator->getData();
				$arrParam['id_product']=$tblDiadiem->saveItem($arrParam,array('task'=>'admin-add'));
                $tblDiadiemEn->saveItem($arrParam,array('task'=>'admin-add'));
...
//		$this->view->slbPage = $tblArticles->itemInSelectbox($this->_arrParam,array('task'=>'get-page'));
        ...
        $this->view->ItemEn = $tblDiadiemEn->getItem($this->_arrParam,array('task'=>'admin-edit'));
		$this->view->slbCategory = $tblDiadiem->itemInSelectbox($this->_arrParam);
        $this->view->slbPage = $tblCategory->listItem($this->_arrParam, array('task'=>'get-phankhu'));
		 
		//$tblUnits = new Product_Model_Units();
//		$this->view->slbDiadiemUnits = $tblUnits->itemInSelectbox($this->_arrParam, array('task'=>'get-units'));
		...
		//$tblSources = new Product_Model_Sources();
...
//		$this->view->slbMoney = $tblMoney->itemInSelectbox($this->_arrParam, array('task'=>'get-money'));
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
                ...
                $tblDiadiemEn->saveItem($arrParam,array('task'=>'admin-edit'));
