...
class Gallery_Model_Image extends Zend_Db_Table{
	...
	public function listItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-list'){
			...
			->joinLeft('gallery_album AS ga', 'ga.id = gi.album_id',array('ga.name AS album_name','ga.picture AS album_picture'));
		...
		if($options['task'] == 'public-image'){
			if(!empty($arrParam['id'])){
				...
				->join('gallery_album AS ga', 'ga.id = gi.album_id',array('picture AS banner', 'ga.alias AS category_alias'))
				->where('gi.status = ?',1,INTERGER)
				->where('gi.album_id = ?',$arrParam['id'],INTERGER)
				->order('gi.order ASC')
				->order('gi.id DESC');
