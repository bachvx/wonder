...
//			}
		...
		if($options['task'] == 'public-category-image'){
			$select = $db->select()
				->from('gallery_album AS ga',array('*'))
                ->where('ga.cat_id <> ?',1,INTERGER)
				->where('ga.status = ?',1,INTERGER)
				->order('ga.order ASC');
			$result = $db->fetchAll($select);
		}
        if($options['task'] == 'public-category-banner'){
			$select = $db->select()
				->from('gallery_album AS ga',array('*'))
                ->where('ga.id = ?', $arrParam, INTERGER);
			//	->where('ga.status = ?',1,INTERGER)
			//	->order('ga.album_id ASC')
			//	->order('ga.order ASC');
			$result = $db->fetchRow($select);
		}
        if($options['task'] == 'public-category-video'){
			$select = $db->select()
				->from('video AS ga',array('*'))
				->where('ga.status = ?',1,INTERGER)
				->order('ga.module ASC')
				->order('ga.order ASC');
			$result = $db->fetchAll($select);
	...
	public function saveItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-add'){
            ...
            $row->picture_multi_mobile 			= $arrParam['picture_multi_mobile'];
