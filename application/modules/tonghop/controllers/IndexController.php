<?php
class Tonghop_IndexController extends Zendvn_Controller_Action {
	...
	public function init(){
		...
		$this->loadTemplate($template_path, 'template.ini', 'quanngon');
...
//		$tblQuanngon = new Tonghop_Model_Item();
...
//		}else{
//			$this->view->headTitle("Ð?i lý", true);
//		}
//		$this->view->headMeta()->setName('description',"Ð?i lý");
//		$this->view->headMeta()->setName('keywords',"Ð?i lý");
//		$this->view->headMeta()->setName('title',"Ð?i lý");
//		
//		$this->view->Items = $tblQuanngon->listItem($this->_arrParam,array('task'=>'public-daily'));
	...
	public function categoryAction(){
		...
		$this->view->Items = $tblQuanngon->listItem($this->_arrParam,array('task'=>'public-category'));
		$this->view->totalItem = $totalItem = $tblQuanngon->countItem($this->_arrParam,array('task'=>'public-category'));
		
		if($totalItem == 1){
			$this->view->ItemArticle = $tblQuanngon->getItem($this->view->Items[0],array('task'=>'public-detail'));
		}
	...
	public function detailAction(){
		$tblQuanngon = new Quanngon_Model_Item();
		$this->view->Item = $tblQuanngon->getItem($this->_arrParam,array('task'=>'public-detail'));
		...
		$this->view->itemNew = $tblQuanngon->listItem($this->_arrParam,array('task'=>'public-new'));
		$this->view->itemOld = $tblQuanngon->listItem($this->_arrParam,array('task'=>'public-old'));

		$tblQuanngon->saveItem($this->view->Item, array('task' => 'public-hits'));
	}
	...
	public function searchAction() {
		...
		$this->view->Items = $tblQuanngon->listItem($this->_arrParam,array('task'=>'public-search'));
	
		$totalItem = $this->view->totalItem = $tblQuanngon->countItem($this->_arrParam,array('task'=>'public-search'));
	...
	public function searchformAction() {
		$this->view->Title = 'Tin quán ăn được tìm thấy theo yêu cầu';
		$this->view->headTitle($this->view->Title, true);
		
		$tblQuanngon = new Quanngon_Model_Item();
	
		$this->view->Items = $tblQuanngon->listItem($this->_arrParam,array('task'=>'public-block', 'block' => 'search'));
		$this->view->totalItem = $totalItem = $tblQuanngon->countItem($this->_arrParam,array('task'=>'public-block', 'block' => 'search'));
