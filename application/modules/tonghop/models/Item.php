<?php
class Tonghop_Model_Item extends Zend_Db_Table{
    protected $_name = 'tuyendung';
	...
	public function getItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-info' || $options['task'] == 'admin-edit'){
			...
			$select = $db->select()
			->from('Quanngon AS a')
			->joinLeft('Quanngon_category AS ac', 'a.cat_id = ac.id',array('ac.name AS category_name'))
			->joinLeft('districts AS di', 'a.districts_id = di.id',array('di.name AS districts_name'))
		...
		if($options['task'] == 'public-category'){
			if(!empty($arrParam['cid'])){
				...
				//$db = Zend_Db::factory($adapter, $config);
				$select = $db->select()
				->from('Quanngon_category AS ac',array('id','name','alias','meta_title','meta_description','meta_keywords'))
				->where('ac.id = ?', $arrParam['cid'], INTEGER);
				$result = $db->fetchRow($select);
			}else{
				$language = Zend_Registry::get('language');
				$result = array('name' => $language['language']['Quanngon'],
						'meta_title' => $language['language']['Quanngon'],
						'meta_description' => $language['language']['Quanngon'],
						'meta_keywords' => $language['language']['Quanngon']
				);
			}
		}
	...
	public function countItem($arrParam = null, $options = null){
		...
		if($options == null){
			...
			//$db = Zend_Db::factory($adapter, $config);
			$select = $db->select()
			->from('tuyendung AS a', array('COUNT(a.id) AS totalItem'));
		...
		if($options['task'] == 'admin-list'){
			...
			$select = $db->select()
			->from('Quanngon AS a', array('COUNT(a.id) AS totalItem'));
			...
			if($ssFilter['cat_id']>0){
				...
				->from('Quanngon_category',array('id','name','parents'))
		...
		if($options['task'] == 'public-block'){
			...
			if(!empty($arrParam['districts_id'])){
				$select->where("a.districts_id = '" . $arrParam['districts_id'] . "'");
			}
			if(!empty($arrParam['keywords'])){
				$select->where("(a.name like '%" . $arrParam['keywords'] . "%') OR (a.alias like '%" . $arrParam['keywords'] . "%')");
			}
			if(!empty($ssFilter['lang_code'])){
				$select->where("a.lang_code = '" . $ssFilter['lang_code'] . "'");
			}
	...
	public function itemInSelectbox($arrParam = null, $options = null){
		...
		if($options == null){
			$select = $db->select()
			->from('Quanngon_category AS ac',array('id','name','parents','status','order','created_by'))
			->order('ac.order ASC')
			->order('ac.id ASC');
			if(!empty($ssFilter['lang_code'])){
	...
	public function listItem($arrParam = null, $options = null){
	   die("das");
		...
		if($options['task'] == 'admin-list'){
			...
			$select = $db->select()
			->from('tuyendung AS a',array('*'))
			//->joinLeft('Quanngon_category AS ac', 'ac.id = a.cat_id',array('ac.name AS category_name'))
			//->joinLeft('districts AS di', 'di.id = a.districts_id',array('di.name AS districts_name'));
		...
		if($options['task'] == 'public-category'){
			if(!empty($arrParam['cid'])){
				...
				$paginator = $arrParam['paginator'];
				$select = $db->select()
				->from('Quanngon AS a',array('id','name','alias','picture','thumb','synopsis','cat_id','created','hits'))
				->join('Quanngon_category AS ac', 'ac.id = a.cat_id',array('name AS category_name', 'ac.alias AS category_alias'))
				->join('districts AS di', 'a.districts_id = di.id',array('di.id AS districts_id','di.name AS districts_name'))
				->where('a.status = ?',1,INTERGER)
		...
		if($options['task'] == 'public-block'){
			...
			if(!empty($arrParam['cid'])){
				...
				->join('districts AS di', 'a.districts_id = di.id',array('di.id AS districts_id','di.name AS districts_name'))
				->where('a.status = ?',1,INTERGER)
				->where('a.cat_id IN (' . $arrParam['cid'] . ')')
				->order('id DESC');
			}else{
				$paginator = $arrParam['paginator'];
				$select = $db->select()
