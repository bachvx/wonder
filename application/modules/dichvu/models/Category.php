<?php
class Dichvu_Model_Category extends Zend_Db_Table{
	
	protected $_name = 'dichvu_category';
	...
	public function itemInSelectbox($arrParam = null, $options = null){
		...
		if($options == null){
			$select = $db->select()
			->from('dichvu_category AS pc',array('id','name','parents','status','order','created_by'))
			->order('pc.order ASC')
			->order('pc.id ASC');
	...
	public function countItem($arrParam = null, $options = null){
		...
		$ssFilter  = $arrParam['ssFilter'];
		$select = $db->select()
		->from('dichvu_category AS pc', array('COUNT(pc.id) AS totalItem'));
	...
	public function listItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-list'){
			$select = $db->select()
			->from('dichvu_category AS pc',array('id','name','parents','status','order','created_by','lang_code'))
		...
		if($options['task'] == 'public-subcategories'){
			$select = $db->select()
			->from('dichvu_category AS pc',array('id','name','alias','parents','picture'))
			->order('pc.order ASC')
			->order('pc.id ASC');
	...
	public function saveItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-add'){
			...
			$row->block_body2 		= $arrParam['block_body2'];
	...
	public function deleteItem($arrParam = null, $options = null){
		if($options['task'] == 'admin-delete'){
			...
			->from('dichvu AS p', array('COUNT(p.id) AS totalItem'))
			->where('cat_id IN (' . $tmp . ')');
