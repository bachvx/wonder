<?php
class Default_Model_Tuyendungen extends Zend_Db_Table{
	
	protected $_name = 'tuyendung_en';
...
//			}
	...
	public function countItem($arrParam = null, $options = null){
		...
		if($options == null){
			...
			$language = new Zend_Session_Namespace('language');
			$select = $db->select()
			->from('tuyendung AS p', array('COUNT(p.id) AS totalItem'))
            //->where('lang_code=?',$language->lang)
            ->where('status=?',1);
...
// 		}
	...
	public function saveItem($arrParam = null, $options = null){
		if($options['task'] == 'admin-add'){
			...
			$row 				= $this->fetchNew();
            $row->id 				= stripslashes($arrParam['id_last']);
			$row->name 				= stripslashes($arrParam['name_en']);
            $row->alias 			= $arrParam['alias'];
			$row->mota 			= $arrParam['mota_en'];
			$row->quyenloi		= $arrParam['quyenloi_en'];
			$row->lienhe 			= stripslashes($arrParam['lienhe_en']);
            $row->trinhdo 			= stripslashes($arrParam['trinhdo_en']);
			$row->gioitinh	 			= $arrParam['gioitinh_en'];
            $row->noilamviec 			= $arrParam['noilamviec_en'];
            $row->mucluong	 			= $arrParam['mucluong_en'];
