<?php
class Default_Model_Tuyendung extends Zend_Db_Table{
	...
	public function getItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'public-detail'){
			...
			$select = $db->select()
			->from('tuyendung AS p')
			->where('p.lang_code = ?', $language->lang)
			->where('p.id = ?', $arrParam['id'], INTEGER);
			// echo $select;die;
			$result = $db->fetchRow($select);
...
// 		}
	...
	public function saveItem($arrParam = null, $options = null){
		if($options['task'] == 'admin-add'){
			...
			$row->quyenloi		= $arrParam['quyenloi'];
			$row->lienhe 			= stripslashes($arrParam['lienhe']);
            $row->trinhdo 			= stripslashes($arrParam['trinhdo']);
			$row->gioitinh	 			= $arrParam['gioitinh'];
            $row->noilamviec 			= $arrParam['noilamviec'];
            $row->mucluong	 			= $arrParam['mucluong'];
