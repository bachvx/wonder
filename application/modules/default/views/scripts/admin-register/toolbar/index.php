<?php
	...
	// 4. Sắp xếp - submit
	$linkSortItem = $this->baseUrl ( $this->currentController . '/sort' . $page);
	...
	// 11. Excel - Link
	$linkExcel = $this->baseUrl ( $this->currentController . '/excel' );
	$btnExcel = $this->cmsButton ( 'Xuất dữ liệu', $linkExcel, $this->imgUrl . '/toolbar/icon-32-export.png', 'link' );
	
	$divider = '<div class="divider"> </div>';
	
	switch ($this->arrParam['action']){
		
		case 'index': $strBtn = $btnExcel . $divider . $btnActiveItems . ' ' . $btnInactiveItems . $divider . $btnDeleteItems;
					 			break;
