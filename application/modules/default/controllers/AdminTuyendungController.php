<?php
class AdminTuyendungController extends Zendvn_Controller_Action{
	...
	public function init(){
		...
		//var_dump($ssFilter->lang_code);die;
	...
	public function indexAction(){		
		$this->view->Title = 'Tuyển dụng :: Index';
	...
	public function addAction(){
		$this->view->Title = 'Tuyển dụng :: Thêm mới';
		$this->view->headTitle($this->view->Title, true);
		
		$tblGroup = new Default_Model_Tuyendung();
        $tblGroup_en = new Default_Model_Tuyendungen();
	...
	//	$this->view->PrivilegesModule = $tblGroup->privileges($this->_arrParam,array('task'=>'admin-module'));
		...
		if($this->_request->isPost()){
			$validator = new Default_Form_ValidateGenex($this->_arrParam);
			if($validator->isError() == true){
				$this->view->messageError = $validator->getMessageError();
				$this->view->Item = $validator->getData();
			}else{
				$arrParam = $validator->getData();
				$arrParam['id_last']=  $tblGroup->saveItem($arrParam,array('task'=>'admin-add'));
                $tblGroup_en->saveItem($arrParam,array('task'=>'admin-add'));
               // $this->_arrParam
				$this->_redirect($this->_actionMain);
			}
		}
	...
	public function editAction(){
	  // die("đa");
		$this->view->Title = 'Tuyển dụng :: Sửa';
		$this->view->headTitle($this->view->Title, true);
		$tblGroup = new Default_Model_Tuyendung();
        $tblGroup_en = new Default_Model_Tuyendungen();
		$this->view->Item = $tblGroup->getItem($this->_arrParam,array('task'=>'admin-edit'));
        $this->view->ItemEn = $tblGroup_en->getItem($this->_arrParam,array('task'=>'admin-edit'));
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
               ...
               // die("1");
				...
				//$tblGroup = new Default_Model_Genexhome();
				$arrParam = $validator->getData();
                //die("2");
				$tblGroup->saveItem($arrParam,array('task'=>'admin-edit'));
                $tblGroup_en->saveItem($arrParam,array('task'=>'admin-edit'));
	...
	public function deleteAction(){
		$this->view->Title = 'Tuyển dụng :: Xóa';
		$this->view->headTitle($this->view->Title, true);
		if($this->_request->isPost()){
            $tblGroup = new Default_Model_Tuyendung();
            $tblGroup_en = new Default_Model_Tuyendungen();
			if(($this->_arrParam['type'] == 'multi-delete') && ($this->_arrParam['task'] == 'ok')){
                ...
                $tblGroup_en->deleteItem($this->_arrParam, array('task'=>'admin-delete-muti'));
				$this->_redirect($this->_actionMain . $this->_page);
			}else
			if(!empty($this->_arrParam['id'])){
				$tblGroup->deleteItem($this->_arrParam, array('task'=>'admin-delete'));
                $tblGroup_en->deleteItem($this->_arrParam, array('task'=>'admin-delete'));
				$this->_redirect($this->_actionMain . $this->_page);
			}
		}
	}
