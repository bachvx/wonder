<?php
class AdminDistrictsController extends Zendvn_Controller_Action{
	...
	public function init(){
		...
		$this->_arrParam['ssFilter']['city_id'] 	= $ssFilter->city_id;
	...
	public function indexAction(){
		...
		$this->view->Items = $tblDistricts->listItem($this->_arrParam, array('task'=>'admin-list'));
		
		$totalItem = $tblDistricts->countItem($this->_arrParam, array('task'=>'admin-list'));
	...
	public function filterAction(){
		...
		if($this->_arrParam['type'] == 'city'){
			$ssFilter->city_id = $this->_arrParam['city_id'];
		}
	...
	public function addAction(){
		$this->view->Title = 'Quận Huyện :: Thêm mới';
		$this->view->headTitle($this->view->Title, true);
...
// 		$this->view->slbCountries = $tblCountry->itemInSelectbox($this->_arrParam, array('task'=>'get-countries'));
		...
		if($this->_request->isPost()){
			...
			$validator = new Default_Form_ValidateDistricts($this->_arrParam);
			if($validator->isError() == true){
				$this->view->messageError = $validator->getMessageError();
				$this->view->Item = $validator->getData();
			}else{
				$arrParam = $validator->getData();
				$tblDistricts->saveItem($arrParam,array('task'=>'admin-add'));
				$this->_redirect($this->_actionMain);
			}
		}
	}
	...
	public function editAction(){
		$this->view->Title = 'Quận Huyện :: Sửa';
		$this->view->headTitle($this->view->Title, true);
...
// 		$this->view->slbCountries = $tblCountry->itemInSelectbox($this->_arrParam, array('task'=>'get-countries'));
		...
		$tblDistricts = new Default_Model_Districts();
		$this->view->Item = $tblDistricts->getItem($this->_arrParam,array('task'=>'admin-edit'));
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
				...
				$arrParam = $validator->getData();
				$tblDistricts->saveItem($arrParam,array('task'=>'admin-edit'));
	...
	public function deleteAction(){
		$this->view->Title = 'Quận Huyện :: Xóa';
		$this->view->headTitle($this->view->Title, true);
		if($this->_request->isPost()){
			if(($this->_arrParam['type'] == 'multi-delete') && ($this->_arrParam['task'] == 'ok')){
				$tblCities = new Default_Model_Districts();
