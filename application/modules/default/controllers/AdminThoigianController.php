<?php
class AdminThoigianController extends Zendvn_Controller_Action{
	...
	public function indexAction(){
		$this->view->Title = 'Thời gian :: Danh sách';
		$this->view->headTitle($this->view->Title, true);
		
		$tblThoigian = new Default_Model_Thoigian();
		$this->view->Items = $tblThoigian->listItem($this->_arrParam, array('task'=>'admin-list'));
		
		$tblLanguages = new Default_Model_Language();
		$this->view->slbLanguages = $tblLanguages->itemInSelectbox($this->_arrParam);
		
		$totalItem = $tblThoigian->countItem($this->_arrParam, array('task'=>'admin-list'));
	...
	public function addAction(){
		$this->view->Title = 'Thời gian :: Thêm mới';
		$this->view->headTitle($this->view->Title, true);
		
		$tblThoigian = new Default_Model_Thoigian();
		$this->view->slbCategory = $tblThoigian->itemInSelectbox($this->_arrParam);
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
				...
				$tblThoigian->saveItem($arrParam,array('task'=>'admin-add'));
				$this->_redirect($this->_actionMain);
			}
		}
	}
	...
	public function editAction(){
		$this->view->Title = 'Thời gian :: Sửa';
		$this->view->headTitle($this->view->Title, true);
		
		$tblThoigian = new Default_Model_Thoigian();
		$this->view->Item = $tblThoigian->getItem($this->_arrParam,array('task'=>'admin-edit'));
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
				...
				$arrParam = $validator->getData();
				$tblThoigian->saveItem($arrParam,array('task'=>'admin-edit'));
	...
	public function deleteAction(){
		$this->view->Title = 'Thời gian :: Xóa';
		$this->view->headTitle($this->view->Title, true);
		if($this->_request->isPost()){
			if(($this->_arrParam['type'] == 'multi-delete') && ($this->_arrParam['task'] == 'ok')){
				$tblThoigian = new Default_Model_Thoigian();
				$tblThoigian->deleteItem($this->_arrParam, array('task'=>'admin-delete-muti'));
				$this->_redirect($this->_actionMain . $this->_page);
			}else
				if(!empty($this->_arrParam['id'])){
				$tblThoigian = new Default_Model_Thoigian();
				$tblThoigian->deleteItem($this->_arrParam, array('task'=>'admin-delete'));
				$this->_redirect($this->_actionMain . $this->_page);
			}
		}
	}
	...
	public function statusAction(){
		$tblThoigian = new Default_Model_Thoigian();
		$tblThoigian->changeStatus($this->_arrParam);
		$this->_redirect($this->_actionMain . $this->_page);
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function sortAction(){
		if($this->_request->isPost()){
			$tblThoigian = new Default_Model_Thoigian();
			$tblThoigian->sortItem($this->_arrParam, array('task'=>'admin-sort'));
			$this->_redirect($this->_actionMain . $this->_page);
		}
		$this->_helper->viewRenderer->setNoRender();
	}
