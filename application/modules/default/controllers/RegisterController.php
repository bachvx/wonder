<?php
class RegisterController extends Zendvn_Controller_Action {
	...
	public function init(){
		...
		$this->loadTemplate($template_path, 'template.ini', 'register');
	...
	public function indexAction() {
		...
		$siteConfig = Zend_Registry::get('siteConfig');
		$this->view->headTitle('Hẹn lịch kiểm tra trình độ', true);
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
				...
				$tblRegister->saveItem($this->_arrParam,array('task'=>'public-add'));
				$this->_redirect('?save=ok');
			}
		...
		/* if($this->_arrParam['save'] == 'ok'){
			$scipt = "alert('Thông tin của bạn đã được gửi thành công. Langmaster sẽ liên hệ với bạn trong thời gian sớm nhất')";
