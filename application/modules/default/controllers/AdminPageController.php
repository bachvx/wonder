<?php
class AdminPageController extends Zendvn_Controller_Action{
	...
	public function indexAction(){
		$this->view->Title = 'Phân khu :: Danh sách';
		...
		$totalItem = $tblTrungtam->countItem($this->_arrParam, array('task'=>'admin-list'));
	...
	public function addAction(){
		$this->view->Title = 'Phân khu :: Thêm mới';
		$this->view->headTitle($this->view->Title, true);
		
		$tblTrungtam = new Default_Model_Trungtam();
		$this->view->slbCategory = $tblTrungtam->itemInSelectbox($this->_arrParam);
		
		if($this->_request->isPost()){
				
			$validator = new Default_Form_ValidateTrungtam($this->_arrParam);
			if($validator->isError() == true){
				$this->view->messageError = $validator->getMessageError();
				$this->view->Item = $validator->getData();
			}else{
				$arrParam = $validator->getData();
				$tblTrungtam->saveItem($arrParam,array('task'=>'admin-add'));
				$this->_redirect($this->_actionMain);
			}
		}
	}
	...
	public function editAction(){
		$this->view->Title = 'Phân khu :: Sửa';
		$this->view->headTitle($this->view->Title, true);
		
		$tblTrungtam = new Default_Model_Trungtam();
		$this->view->Item = $tblTrungtam->getItem($this->_arrParam,array('task'=>'admin-edit'));
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
				...
				$tblTrungtam->saveItem($arrParam,array('task'=>'admin-edit'));
	...
	public function deleteAction(){
		$this->view->Title = 'Phân khu :: Xóa';
		$this->view->headTitle($this->view->Title, true);
		if($this->_request->isPost()){
			if(($this->_arrParam['type'] == 'multi-delete') && ($this->_arrParam['task'] == 'ok')){
				$tblTrungtam = new Default_Model_Trungtam();
				$tblTrungtam->deleteItem($this->_arrParam, array('task'=>'admin-delete-muti'));
				$this->_redirect($this->_actionMain . $this->_page);
			}else
				if(!empty($this->_arrParam['id'])){
				$tblTrungtam = new Default_Model_Trungtam();
				$tblTrungtam->deleteItem($this->_arrParam, array('task'=>'admin-delete'));
				$this->_redirect($this->_actionMain . $this->_page);
			}
		}
	}
	...
	public function statusAction(){
		$tblTrungtam = new Default_Model_Trungtam();
		$tblTrungtam->changeStatus($this->_arrParam);
		$this->_redirect($this->_actionMain . $this->_page);
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function sortAction(){
		if($this->_request->isPost()){
			$tblTrungtam = new Default_Model_Trungtam();
			$tblTrungtam->sortItem($this->_arrParam, array('task'=>'admin-sort'));
			$this->_redirect($this->_actionMain . $this->_page);
		}
		$this->_helper->viewRenderer->setNoRender();
	}
