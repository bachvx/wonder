<?php
class AdminPhanphoiController extends Zendvn_Controller_Action{
	...
	public function indexAction(){
		...
		$this->view->Title = 'Đại lý phân phối :: Danh sách';
		$this->view->headTitle($this->view->Title, true);
		
		$tblLink = new Default_Model_Phanphoi();
		$this->view->Items = $tblLink->listItem($this->_arrParam, array('task'=>'admin-list'));
 
		$this->view->slbCategory = $tblLink->itemInSelectbox($this->_arrParam);
	//	
//		$tblLanguages = new Default_Model_Language();
//		$this->view->slbLanguages = $tblLanguages->itemInSelectbox($this->_arrParam);
		
		$totalItem = $tblLink->countItem($this->_arrParam, array('task'=>'admin-list'));
	...
	public function addAction(){
		$this->view->Title = 'Liên kết :: Thêm mới';
		$this->view->headTitle($this->view->Title, true);
		
		$tblLink = new Default_Model_Phanphoi();
		$this->view->slbCategory = $tblLink->itemInSelectbox($this->_arrParam);
		$this->view->Mien = $tblLink->getOther($this->_arrParam,array('task'=>'get-mien'));
		if($this->_request->isPost()){
				
			$validator = new Default_Form_ValidatePhanphoi($this->_arrParam);
	...
	public function infoAction(){
		$this->view->Title = 'Liên kết :: Xem thông tin';
		$this->view->headTitle($this->view->Title, true);
		$tblLink = new Default_Model_Link();
		$this->view->Item = $tblLink->getItem($this->_arrParam,array('task'=>'admin-info'));
		
	}
	
	public function editAction(){
		$this->view->Title = 'Phân phối :: Sửa';
		$this->view->headTitle($this->view->Title, true);
	...
	public function deleteAction(){
		$this->view->Title = 'Phân phối :: Xóa';
		$this->view->headTitle($this->view->Title, true);
		if($this->_request->isPost()){
			if(($this->_arrParam['type'] == 'multi-delete') && ($this->_arrParam['task'] == 'ok')){
				$tblLink = new Default_Model_Phanphoi();
				$tblLink->deleteItem($this->_arrParam, array('task'=>'admin-delete-muti'));
				$this->_redirect($this->_actionMain . $this->_page);
			}else
	...
	public function statusAction(){
		$tblLink = new Default_Model_Phanphoi();
		$tblLink->changeStatus($this->_arrParam);
		$this->_redirect($this->_actionMain . $this->_page);
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function sortAction(){
		if($this->_request->isPost()){
			$tblLink = new Default_Model_Phanphoi();
			$tblLink->sortItem($this->_arrParam, array('task'=>'admin-sort'));
			$this->_redirect($this->_actionMain . $this->_page);
		}
		$this->_helper->viewRenderer->setNoRender();
	}
