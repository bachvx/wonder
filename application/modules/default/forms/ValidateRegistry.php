	...
	public function __construct($arrParam = array(), $options = null){
		...
		if($options == null){
			...
			if($arrParam['action'] == 'edit'){
				if($arrParam['ngaysinh'] == '00'){
					$this->_messagesError['member_ngaysinh'] = 'Vui lòng chọn ngày, tháng, năm sinh';
				}else{
					if($arrParam['thangsinh'] == '00'){
						$this->_messagesError['member_ngaysinh'] = 'Vui lòng chọn ngày, tháng, năm sinh';
					}else{
						if($arrParam['namsinh'] == '00'){
							$this->_messagesError['member_ngaysinh'] = 'Vui lòng chọn ngày, tháng, năm sinh';
						}
					}
				}
			...
			//=========================================
			//KIEM TRA member_diachi
			//=========================================
			if($arrParam['action'] == 'edit'){
				$validator = new Zend_Validate();
				$validator->addValidator(new Zend_Validate_NotEmpty(),true)
							->addValidator(new Zend_Validate_StringLength(2,250),true);
				if(!$validator->isValid($arrParam['member_diachi'])){
					$message = $validator->getMessages();
					$this->_messagesError['member_diachi'] = current($message);
					$arrParam['member_diachi'] = '';
				}
			}
			//=========================================
			//KIEM TRA tinh
			//=========================================
			if($arrParam['action'] == 'edit'){
				if($arrParam['member_tinh'] == 0){
					$this->_messagesError['member_tinh'] = 'Vui lòng chọn tỉnh/thành phố';
				}
			}
			//=========================================
...
// 				$validator->addValidator(new Zend_Validate_NotEmpty(),true)
// 				->addValidator(new Zend_Validate_EmailAddress(),true);
// 				if(!$validator->isValid($arrParam['member_email'])){
// 					$message = $validator->getMessages();
// 					$this->_messagesError['member_email'] = current($message);
// 					$arrParam['member_email'] = '';
// 				}
// 			}
// 			//=========================================
			//KIEM TRA Captcha
